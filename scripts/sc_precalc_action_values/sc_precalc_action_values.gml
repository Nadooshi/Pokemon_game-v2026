function sc_precalc_action_values(a_map, pokemon_id){
	if not global.sc_does_exist(a_map, undefined, "'a_map' in sc_precalc_..") or not global.sc_does_exist(pokemon_id, undefined, "'pokemon_id' in sc_precalc_..")
		return false
	...
	case _ATTACK_TYPE.front:
		a_map[? "distance"] = 8 + pokemon_id.pokemon_map[? "size"] * 2
		break;
	case _ATTACK_TYPE.melee:
		a_map[? "distance"] = 8
	break
	case _ATTACK_TYPE.pool:
		a_map[? "distance"] = (a_map[? "radius"] + 1) * 8 + pokemon_id.pokemon_map[? "size"] * 2 + 2
	break;
