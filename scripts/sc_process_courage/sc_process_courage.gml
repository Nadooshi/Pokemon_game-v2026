...
// for ob_player
	...
	var _en_force = 0
	var _fr_force = 0
	var _u_force = (pokemon_map[? "rating"] + pokemon_map[? "level"] + health_cur) * (1 + power_cur * 0.1)
	...
	if id != other.id {
		...
		if _d<= max_distance_attack {
			...
			_fr_force *= 1 + power_cur * 0.01
		}
	}
	var _mod_courage = ((_fr_force + _u_force - _en_force)/ _u_force) * mod_boost
