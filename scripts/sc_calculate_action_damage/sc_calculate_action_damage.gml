function sc_calculate_action_damage(_action, p_id_target, _p_attack) {
	...
	_dmg[0, _e.element ] = 0
	_dmg[1, _e.element ] = 0
	_dmg[0, _e.material] = 0
	_dmg[1, _e.material] = 0

	if not is_nan(_action[? "dmg_element"]){
		_dmg[0, _e.element ] = _action[? "dmg_element"]
		_dmg[1, _e.element ] = _action[? "dmg_element"]
	}
	if not is_nan(_action[? "dmg_material"]){
	...
	if is_nan(potential_damage) show_message("p_damage NaN!")
